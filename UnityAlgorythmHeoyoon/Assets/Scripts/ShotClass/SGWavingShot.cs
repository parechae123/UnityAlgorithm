...
public class SGWavingShot : SGBaseShot
{
    ...
    protected virtual void Update()
    {
        ...
        while (delayTimer <= 0)
        {
            for (int i = 0; i < wayNum; i++)
            {
                ...
                float baseAngle = waveSpeed % 2 == 0 ? centerAngle - (betweenAngle / 2f) : centerAngle;
    ...
    public float GetShiftedAngle(int wayIndex, float baseAngle, float betweenAngle)
    {
        float angle = wayIndex % 2 == 0 ?
            baseAngle - (this.betweenAngle * (float)wayIndex / 2f) :
            baseAngle + (this.betweenAngle * Mathf.Ceil((float)wayIndex / 2f));
        return angle;
    }
}
