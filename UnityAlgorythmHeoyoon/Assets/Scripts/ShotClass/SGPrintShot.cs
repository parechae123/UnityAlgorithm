...
public class SGPrintShot : SGBaseShot
{
    ...
    public override void Shot()
    {
        ...
        {//파일 텍스트 검사 , 테스트 파일 안에 string이 있는지도 검사
            return;
        }
    ...
    private List<List<int>> LoadPaintData()
    {
        ...
        string[] lines = paintDataText.text.Split(SPLIT,System.StringSplitOptions.RemoveEmptyEntries);  //데이터를 잘라 라인별로 입력
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            {
                paintData[paintData.Count - 1].Add(lines[i][j] == 'O' ? 1 : 0);     //스트링값 알파벳 O(오)
            }
        }
